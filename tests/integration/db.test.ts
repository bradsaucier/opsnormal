...
describe('database operations', () => {
  ...
  it('orders exports by compound key and preserves date-range queries', async () => {
    await setDailyStatus('2026-03-28', 'rest', 'nominal');
    await setDailyStatus('2026-03-27', 'body', 'degraded');
    await setDailyStatus('2026-03-28', 'body', 'nominal');

    const allEntries = await getAllEntries();

    expect(allEntries.map((entry) => `${entry.date}:${entry.sectorId}`)).toEqual([
      '2026-03-27:body',
      '2026-03-28:body',
      '2026-03-28:rest'
    ]);

    const sameDayEntries = await db.dailyEntries.where('date').equals('2026-03-28').toArray();

    expect(sameDayEntries.map((entry) => entry.sectorId)).toEqual(['body', 'rest']);
  });
});
