...
describe('ExportPanelCrashFallback', () => {
  ...
  it('keeps emergency export actions available with accessible fallback semantics', async () => {
    ...
    const { container } = render(
      <ExportPanelCrashFallback error={new Error('Export crash')} componentStack={'at ExportPanel'} onRetry={vi.fn()} />
    );

    expect(screen.getByRole('alert')).toHaveFocus();

    await user.click(screen.getByRole('button', { name: /emergency json export/i }));

    expect(mocks.exportEmergencyJsonBackup).toHaveBeenCalledTimes(1);
    expect(screen.getAllByText('Emergency JSON export complete. 2 entries recovered.')).toHaveLength(2);

    const alert = screen.getByRole('alert');
    const status = screen.getByRole('status');

    expect(status).toHaveTextContent('Emergency JSON export complete. 2 entries recovered.');
  ...
  it('disables recovery controls while an emergency export is running', async () => {
    let resolveExport: ((value: { recoveredCount: number; skippedCount: number; exportedAt: string }) => void) | undefined;
    ...
    render(<ExportPanelCrashFallback error={new Error('Export crash')} onRetry={vi.fn()} />);

    const jsonButton = screen.getByRole('button', { name: /emergency json export/i });
    const csvButton = screen.getByRole('button', { name: /emergency csv export/i });
  ...
  it('surfaces manual inspection guidance if emergency export fails', async () => {
    mocks.exportEmergencyJsonBackup.mockRejectedValueOnce(new Error('Dexie open failed'));
    ...
    expect(screen.getByText(/inspect browser devtools, then application, then indexeddb/i)).toBeInTheDocument();
  });

  it('calls retry and reload actions', async () => {
    const onRetry = vi.fn();

    render(<ExportPanelCrashFallback error={new Error('Export crash')} onRetry={onRetry} />);
