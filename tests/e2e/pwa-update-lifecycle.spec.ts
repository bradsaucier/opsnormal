...
async function queueForegroundUpdateReady(
...
async function getForegroundRevalidationCount(
  page: import('@playwright/test').Page,
) {
  return page.evaluate(
    () => window.__opsNormalPwaTestApi__?.getForegroundRevalidationCount() ?? 0,
  );
}

async function dispatchSyntheticForegroundReturn(
  page: import('@playwright/test').Page,
) {
  await page.evaluate(() => {
...
test.describe('OpsNormal PWA update lifecycle', () => {
  test('surfaces a queued update on a synthetic foreground return without repeated revalidation churn @harness', async ({
    browser,
  }) => {
    ...
    await expect(
      pageA.getByRole('heading', { name: 'Update Ready' }),
    ).toBeVisible();
  ...
  test('reloads both tabs through the synthetic controller handoff without losing the visible check-in state @harness', async ({
    browser,
  }) => {
    const context = await browser.newContext({ serviceWorkers: 'block' });
    const appUrl = 'http://127.0.0.1:4173/';
    ...
    const workNominal = pageA.getByRole('radio', {
    ...
    const versionChangeResult = await pageB.evaluate(
      () => window.__opsNormalDbTestApi__?.simulateVersionChange() ?? 'noop',
    );
    ...
    const pageANavigationType = await pageA.evaluate((): string => {
      const lastNavigationEntry = performance
    ...
    await expect(
      pageA.getByRole('heading', { name: 'OpsNormal' }),
    ).toBeVisible();
    await expect(
      pageB.getByRole('heading', { name: 'OpsNormal' }),
    ).toBeVisible();
    await expect(
      pageA.getByRole('radio', { name: /^Work or School nominal$/i }),
    ).toHaveAttribute('aria-checked', 'true');
    await expect
      .poll(() =>
        pageA.evaluate(
          () => window.__opsNormalDbTestApi__?.isRecoveryRequired() ?? true,
        ),
      )
      .toBe(false);
    await expect
  ...
  test('pins loop-breaker guidance after repeated automatic reload bookkeeping is detected', async ({
    page,
  }) => {
    ...
    await page.evaluate(() => {
      window.sessionStorage.setItem(
        'opsnormal-sw-controller-reload-count',
        '2',
      );
      window.sessionStorage.setItem(
    ...
    await expect(
      updateBanner.getByRole('heading', { name: 'Update Recovery Required' }),
    ).toBeVisible();
    await expect(
      updateBanner.getByText(/update loop intercepted/i),
    ).toBeVisible();
    await expect(
      updateBanner.getByRole('button', { name: /reload tab/i }),
    ).toBeVisible();
    await expect(
      updateBanner.getByRole('button', { name: /dismiss/i }),
    ).toHaveCount(0);
  });

  test('clears stale recovery guidance in a second tab when manual recovery starts', async ({
    browser,
  }) => {
    const context = await browser.newContext({ serviceWorkers: 'block' });
    const appUrl = 'http://127.0.0.1:4173/';
    ...
    for (const page of [pageA, pageB]) {
      ...
      await page.reload({ waitUntil: 'domcontentloaded' });
      await expect(
        getPwaUpdateBanner(page).getByRole('heading', {
          name: 'Update Recovery Required',
        }),
      ).toBeVisible();
    }
    ...
    await expect(
      pageB.getByRole('heading', { name: 'Update Recovery Required' }),
    ).toHaveCount(0);
    await expect
      .poll(() =>
        pageB.evaluate(() =>
  ...
  test('pins stalled handoff guidance until the operator reloads @harness', async ({
    page,
  }) => {
    await page.goto('http://127.0.0.1:4173/');
    ...
    await expect(
      updateBanner.getByText(/update handoff did not complete/i),
