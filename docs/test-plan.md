...
- narrow WebKit smoke coverage proves app boot, Safari-family warning rendering, and IndexedDB persistence without claiming eviction simulation
...
Safari storage lifecycle coverage is intentionally split across three layers. Unit tests prove storage-health and backup-prompt decision logic. Chromium e2e harness tests inject synthetic storage states and verify the exact operator-facing warning surfaces. A narrow WebKit smoke lane proves rendering and IndexedDB I-O on a WebKit engine, but it does not claim to reproduce Safari's seven-day purge behavior.
