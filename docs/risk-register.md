...
- Mitigation: surface the exact platform risk explicitly, recommend Home Screen installation on Apple devices, request persistent storage without implying a guarantee, keep export available, raise a fresh-backup prompt before the seven-day window is likely to close, unit-test the storage-health decision tree, cover the warning UI through the synthetic Chromium harness, and prove page-side service-worker readiness plus offline shell retrieval after first controlled load in the merge-blocking WebKit smoke lane. That lane remains compatibility coverage and does not claim to simulate eviction.
